...
const OfficeLabourForm = () => {
  ...
  // ── Table Filtering (✅ STATUS 'DONE/REJECT' HIDE LOGIC ADDED) ──
  const filteredRequirements = useMemo(() => {
    if (!requirementsData) return [];
    
    // 1️⃣ Pehle STATUS (Column Q) ke base par filter karo
    let data = requirementsData.filter(item => {
      // Backend se aane wala Status (Column Q)
      const statusQ = (item.Status || '').toString().trim().toLowerCase();
      
      // Agar 'done' ya 'reject' hai, toh usko list me mat dikhao
      return statusQ !== 'done' && statusQ !== 'reject';
    });

    // 2️⃣ Phir SEARCH BAR ke base par filter karo
    if (searchTerm) {
