...
//   // ── Fetch Account Names from Project_Data!AB ──────────
//   const fetchAccountNames = async () => {
//     try {
//       setAccountLoading(true);
//       const response = await axios.get(
//         `${import.meta.env.VITE_BACKEND_URL}/api/account-names`
//       );
...
//       setLoading(true);
//       const response = await axios.get(
//         `${import.meta.env.VITE_BACKEND_URL}/api/Payment_15`
//       );
//       if (response.data.success) {
...
//     return (
//       Number(bill.netAmount16 || 0) -
//       Number(bill.latestPaidAmount || 0) -
...
//     return {
//       netAmount, previousPaid, advanceAmount, currentBalance,
//       currentPaid, effectivePaid, newBalance,
//       totalPaidAfter: totalDeducted,
//     };
//   };
...
//       return;
//     }
//     const zeroPaidBills = selectedBills.filter(
...
//               fontWeight: 600, color: T.navyLight, marginBottom: 6,
//             }}>
//               Vendor
//             </label>
...
//                 {vendors
//                   .filter(v =>
//                     v.vendorFirmName.toLowerCase().includes(vendorSearch.toLowerCase())
//                   )
//                   .map((v, i) => (
//                     <li
//                       key={i}
...
//                     </div>
//                     <div>
//                       <h3 style={{ margin: 0, color: T.navy, fontSize: 16 }}>
//                         {item.siteName}
//                       </h3>
...
//                     <DetailItem label="Advance" value={`Rs.${summary.advanceAmount.toLocaleString("en-IN")}`} color="#e65100" />
//                   )}
//                   <DetailItem
//                     label="Balance"
//                     value={`Rs.${summary.currentBalance.toLocaleString("en-IN")}`}
//                     color={summary.currentBalance <= 0 ? T.danger : T.success}
//                   />
//                 </div>
...
//                     background: T.warningBg, borderRadius: 8,
...
//                       background: `${T.navy}08`, borderRadius: 8,
...
//                       }}>
//                         <div>
//                           <span style={{ color: T.textLight }}>Net Amount:</span>{" "}
//                           <strong style={{ color: T.navy }}>
//                             Rs.{summary.netAmount.toLocaleString("en-IN")}
//                           </strong>
//                         </div>
//                         <div>
//                           <span style={{ color: T.textLight }}>Previously Paid:</span>{" "}
//                           <strong style={{ color: T.success }}>
//                             Rs.{summary.previousPaid.toLocaleString("en-IN")}
//                           </strong>
//                         </div>
//                         {summary.advanceAmount > 0 && (
//                           <div>
//                             <span style={{ color: T.textLight }}>Advance:</span>{" "}
//                             <strong style={{ color: '#e65100' }}>
//                               Rs.{summary.advanceAmount.toLocaleString("en-IN")}
//                             </strong>
//                           </div>
//                         )}
//                         <div>
//                           <span style={{ color: T.textLight }}>Balance:</span>{" "}
//                           <strong style={{ color: T.danger }}>
//                             Rs.{summary.currentBalance.toLocaleString("en-IN")}
//                           </strong>
//                         </div>
//                         <div>
//                           <span style={{ color: T.textLight }}>New Balance:</span>{" "}
//                           <strong style={{ color: summary.newBalance === 0 ? T.success : T.danger }}>
//                             Rs.{summary.newBalance.toLocaleString("en-IN")}
//                           </strong>
//                         </div>
//                       </div>
//                     </div>

//                     {/* Paid / Round-Off / New Balance Inputs */}
//                     <div style={{
...
//                           color: T.navyLight, display: 'block', marginBottom: 4,
//                         }}>
//                           PAID AMOUNT
//                         </label>
//                         <input
...
//                       background: T.border, borderRadius: 3, overflow: 'hidden',
//                     }}>
//                       <div style={{
...
//                       type="text"
//                       placeholder={
//                         accountLoading ? "Loading accounts..." : "Search or Select Bank..."
//                       }
//                       value={bankSearch}
...
//                       </span>
//                       <button
//                         onClick={() => { setBankDetails(""); setBankSearch(""); }}
//                         style={{
//                           background: 'none', border: 'none',
//                           cursor: 'pointer', color: T.danger, fontSize: 11,
//                           display: 'flex', alignItems: 'center',
//                         }}
//                       >
//                         <X size={11} />
...
//                               borderBottom: `1px solid ${T.borderLight}`,
//                               fontSize: 13,
//                               background: bankDetails === name
//                                 ? `${T.gold}15` : 'transparent',
//                               color: bankDetails === name ? T.navy : T.text,
...
//                         }}>
//                           No accounts found for "{bankSearch}"
//                         </li>
//                       )}
//                     </ul>
//                   )}
...
//                   <input
//                     type="text"
//                     placeholder={paymentMode === "Cheque" ? "Cheque Number" : "Details"}
...
//   <div>
//     <div style={{ fontSize: 10, color: '#94a3b8', fontWeight: 700, marginBottom: 3 }}>
//       {label}
//     </div>
...
const Payment15 = () => {
      ...
      if (response.data.success) {
        const processedData = response.data.data.map((item) => {
          ...
          return {
            ...
            latestPaidAmount: latestPaid,
            advanceAmount: advanceAmount,
            latestBalanceAmount: rawBalance > 0 ? rawBalance : 0,
          };
        });
        setAllData(processedData);
        setVendors(response.data.uniqueVendors);
  ...
  const handleFilter = (freshData = null) => {
    ...
    const results = sourceData.filter((item) => {
      const vendorMatch = item.vendorFirmName?.trim().toLowerCase() === currentVendor.toLowerCase();
      const siteMatch = (currentSite && currentSite !== "")
        ? item.siteName?.trim().toLowerCase() === currentSite.toLowerCase() : true;
      return vendorMatch && siteMatch;
    });
    if (results.length === 0 && currentSite) {
      const vendorOnlyResults = sourceData.filter(
  ...
  const handlePaidAmountChange = (uid, value) => {
    ...
    const bill = filteredData.find((b) => b.UID === uid);
    if (!bill) return;
    if (isFullyPaidByAdvance(bill)) { numValue = 0; }
    else {
      const currentBalance = calculateCurrentBalance(bill);
      if (numValue > currentBalance) {
        alert(`Cannot pay more than balance (Rs.${currentBalance.toLocaleString("en-IN")})`);
  ...
  // ✅ Check all selected are advance
 ...
 const handleSubmit = async () => {
  ...
  let advanceBillCount = 0;
  let normalBillCount = 0;
  selectedBills.forEach((uid) => {
    const bill = filteredData.find((b) => b.UID === uid);
    if (bill) {
      const net = Number(bill.netAmount16 || 0);
      const paid = Number(bill.latestPaidAmount || 0);
      const advance = Number(bill.advanceAmount || 0);
      if ((paid + advance) >= net) advanceBillCount++;
      else normalBillCount++;
    }
  });

  if (normalBillCount > 0) {
    let missing = [];
    if (!bankDetails) missing.push("Bank");
    if (!paymentMode) missing.push("Payment Mode");
    if (!paymentDetails.trim()) missing.push("Payment Details");
    if (!paymentDate) missing.push("Payment Date");
    if (missing.length > 0) {
  ...
  const emptyPaidBills = selectedBills.filter((uid) => {
    ...
    const paid = Number(bill.latestPaidAmount || 0);
    const advance = Number(bill.advanceAmount || 0);
    if ((paid + advance) >= net) return false;
    const paidVal = paidAmounts[uid];
    const roundVal = roundups[uid];
    return (
      paidVal === undefined || paidVal === null || paidVal === "" || isNaN(paidVal) ||
      roundVal === undefined || roundVal === null || roundVal === "" || isNaN(roundVal)
    );
  });
  if (emptyPaidBills.length > 0) {
    alert(`${emptyPaidBills.length} bill(s) mein Paid Amount ya Round Off khali hai.`);
  ...
  try {
    const payload = selectedBills.map((uid) => {
      ...
      const paid = Number(bill.latestPaidAmount || 0);
      const advance = Number(bill.advanceAmount || 0);
      const fullyPaid = (paid + advance) >= net;

      if (fullyPaid) {
        // ✅ Excess advance = remaining after this bill
        const excessAdvance = (paid + advance) - net;
        ...
        return {
          ...
          advanceAmount: summary.advanceAmount,
          isAdvanceBill: true,
          // ✅ Remaining advance after this bill
          remainingAdvance: excessAdvance,
          // ✅ How much advance used for this bill
          advanceUsedForThisBill: summary.effectiveAdvance,
        };
      } else {
        const hColumnValue = summary.effectivePaid;
        const paidAmount17 = summary.previousPaid + summary.effectivePaid;
    ...
    if (response.data.success) {
      alert(`Success! ${response.data.addedToPaymentSheet} payments recorded.`);
      setSelectedBills([]); setPaidAmounts({}); setRoundups({});
      setBankDetails(""); setBankSearch("");
      setPaymentMode(""); setPaymentDetails(""); setPaymentDate("");
      const freshData = await fetchInitialData();
      if (freshData) handleFilter(freshData);
    } else {
      alert("Error: " + (response.data.error || "Something went wrong"));
  ...
  if (loading) return (
    <div style={{
      ...
      alignItems: 'center', minHeight: '60vh', fontFamily: 'sans-serif',
    }}>
      <div style={{
        width: 56, height: 56, borderRadius: 14,
        background: `linear-gradient(135deg, ${T.navy}, ${T.navyLight})`,
    ...
    <div style={{ maxWidth: 1400, margin: '0 auto', padding: '0 8px' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{
            ...
            background: `linear-gradient(135deg, ${T.navy}, ${T.navyLight})`,
            display: 'flex', alignItems: 'center', justifyContent: 'center',
          }}><Package size={18} color={T.gold} /></div>
          <div>
            <h2 style={{ fontSize: 16, fontWeight: 700, color: T.navy, margin: 0 }}>Payment — Step 15</h2>
            <p style={{ fontSize: 12, color: T.textMuted, margin: 0 }}>{allData.length} pending bills</p>
          </div>
        </div>
      ...
      <div style={{
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr auto', gap: 12, alignItems: 'end' }}>
          ...
          <div style={{ position: 'relative' }} ref={vendorRef}>
            ...
            {showVendorList && (
              <ul style={{
                position: 'absolute', top: '100%', left: 0, right: 0, background: T.card,
                border: `1px solid ${T.border}`, borderRadius: 8, marginTop: 4, maxHeight: 200,
                overflowY: 'auto', zIndex: 100, listStyle: 'none', padding: 0, boxShadow: '0 4px 12px rgba(0,0,0,0.1)',
              }}>
          ...
          <button onClick={() => handleFilter()} style={{
            ...
            padding: '10px 24px', borderRadius: 8, border: 'none',
            background: `linear-gradient(135deg, ${T.gold}, ${T.goldDark})`,
            color: T.navyDark, fontSize: 13, fontWeight: 700, cursor: 'pointer', height: 42,
            boxShadow: `0 2px 8px ${T.gold}40`,
          }}><Search size={14} /> Filter</button>
        </div>
      </div>
      ...
      {showData && (
        <div>
          <div style={{
            display: 'flex', justifyContent: 'space-between', alignItems: 'center',
            marginBottom: 16, padding: '10px 16px', background: T.card, borderRadius: 8, border: `1px solid ${T.border}`,
          }}>
            <div style={{ fontSize: 18, fontWeight: 700, color: T.navy }}>
              {vendorSearch}
            ...
            <button onClick={() => { setShowData(false); setSelectedBills([]); setPaidAmounts({}); setRoundups({}); }}
              style={{
                ...
                border: `1px solid ${T.danger}40`, background: T.dangerBg, color: T.danger, fontSize: 12, fontWeight: 600, cursor: 'pointer',
              }}><X size={12} /> Change</button>
          </div>
          ...
          {filteredData.map((item) => {
            ...
            return (
              <div key={item.UID} style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
                  <div style={{ display: 'flex', gap: 14, alignItems: 'center' }}>
                    <div onClick={() => toggleBillSelection(item.UID)} style={{
                      border: `2px solid ${isSelected ? T.success : T.border}`, borderRadius: 8, padding: '8px 14px',
                  ...
                  <div style={{ textAlign: 'right' }}>
                    <div style={{ fontSize: 22, fontWeight: 800, color: T.success }}>Rs.{summary.netAmount.toLocaleString("en-IN")}</div>
                ...
                {fullyPaid && (
                  <div style={{
                    marginBottom: 14, padding: '12px 16px', background: '#f3e8ff', borderRadius: 8,
                    borderLeft: `4px solid ${T.purple}`, display: 'flex', alignItems: 'center', gap: 10,
                  }}>
                    <span style={{ fontSize: 20 }}>✅</span>
                    <div>
                      <div style={{ color: '#6b21a8', fontWeight: 700, fontSize: 14 }}>Fully Covered by Advance Payment</div>
                      <div style={{ color: '#7c3aed', fontSize: 12, marginTop: 2 }}>
                        Advance: Rs.{summary.advanceAmount.toLocaleString("en-IN")}
                        {excessAdv > 0 && <span style={{ marginLeft: 8, color: '#059669', fontWeight: 700 }}>| Excess: +Rs.{excessAdv.toLocaleString("en-IN")}</span>}
                      </div>
                    </div>
                  </div>
                )}
                ...
                <div style={{
                  ...
                  {summary.advanceAmount > 0 && (
                    <DetailItem label={fullyPaid ? "ADVANCE (FULL COVER)" : "ADVANCE"}
                      value={`Rs.${summary.advanceAmount.toLocaleString("en-IN")}`} color={fullyPaid ? T.purple : "#e65100"} />
                  )}
                  {summary.advanceAmount > 0 && summary.effectiveAdvance !== summary.advanceAmount && (
                    <DetailItem label="ADVANCE USED" value={`Rs.${summary.effectiveAdvance.toLocaleString("en-IN")}`} color={T.success} />
                  )}
                  {excessAdv > 0 && <DetailItem label="EXCESS ADVANCE" value={`+Rs.${excessAdv.toLocaleString("en-IN")}`} color="#059669" />}
                ...
                {summary.advanceAmount > 0 && !fullyPaid && (
                  <div style={{
                    marginTop: 12, padding: '10px 14px', background: T.warningBg, borderRadius: 8,
                    borderLeft: `3px solid ${T.warning}`, display: 'flex', alignItems: 'center', gap: 8,
                  }}>
                    <span style={{ fontSize: 16 }}>⚡</span>
                    <span style={{ color: '#92400e', fontWeight: 600, fontSize: 13 }}>
                      Advance Rs.{summary.effectiveAdvance.toLocaleString("en-IN")} adjusted (Total: Rs.{summary.advanceAmount.toLocaleString("en-IN")})
                    </span>
                  </div>
                )}

                <div style={{
                  marginTop: 14, display: 'flex', gap: 10, borderTop: `1px solid ${T.border}`, paddingTop: 12, flexWrap: 'wrap',
                }}>
                  {item.finalIndentPDF && <DocLink label="Indent" url={item.finalIndentPDF} />}
                  {item.poPDF && <DocLink label="PO" url={item.poPDF} />}
                  {item.mrnPDF && <DocLink label="MRN" url={item.mrnPDF} />}
                ...
                {isSelected && (
                  <div style={{
                    marginTop: 20, padding: 16,
                    background: fullyPaid ? '#f3e8ff' : T.successBg, borderRadius: 10,
                    border: `1px solid ${fullyPaid ? '#c084fc' : T.successBorder}`,
                  }}>
                    <h4 style={{ margin: '0 0 14px', color: fullyPaid ? '#6b21a8' : '#065f46', fontSize: 14 }}>
                      Payment — <strong>{item.invoice13}</strong> (UID: {item.UID})
                      {fullyPaid && (
                        <span style={{ marginLeft: 10, background: T.purple, color: '#fff', padding: '2px 10px', borderRadius: 12, fontSize: 11 }}>
                    ...
                    <div style={{
                      marginBottom: 14, padding: 12, background: `${T.navy}08`, borderRadius: 8,
                      borderLeft: `3px solid ${fullyPaid ? T.purple : T.navy}`,
                    }}>
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 10, fontSize: 13 }}>
                        ...
                        {summary.advanceAmount > 0 && (
                          <>
                            ...
                            {excessAdv > 0 && <div><span style={{ color: T.textLight }}>Excess:</span> <strong style={{ color: '#059669' }}>+Rs.{excessAdv.toLocaleString("en-IN")}</strong></div>}
                          </>
                        )}
                    ...
                    {/* ✅ Inputs only for non-advance bills */}
                    {!fullyPaid && (
                    ...
                    {!fullyPaid && (
                      <button onClick={scrollToPaymentSection} style={{
                        ...
                        display: 'flex', alignItems: 'center', gap: 6,
                      }}><ArrowDown size={13} /> Go to Payment Section</button>
                    )}
                  </div>
                )}
              </div>
          ...
          {/* Grand Total */}
          {selectedBills.length > 0 && !allSelectedAreAdvance && (
            <div style={{
              background: T.successBg, padding: 16, borderRadius: 10, marginTop: 20,
              border: `2px solid ${T.success}`,
              display: 'flex', justifyContent: 'space-between', alignItems: 'center',
              ...
              <h3 style={{ margin: 0, color: '#065f46', fontSize: 16 }}>Grand Total</h3>
              <span style={{ fontSize: 24, fontWeight: 800, color: T.success }}>Rs.{grandTotal.toLocaleString("en-IN")}</span>
            </div>
          )}
          ...
          {selectedBills.length > 0 && (
            <div ref={paymentSectionRef} style={{
              ...
              {/* ✅ ALL ADVANCE - No fields, direct submit */}
              {allSelectedAreAdvance ? (
                <>
                  <div style={{
                    padding: 24, background: '#f3e8ff', borderRadius: 10,
                    border: `2px solid ${T.purple}`, marginBottom: 16, textAlign: 'center',
                  }}>
                    <span style={{ fontSize: 40 }}>✅</span>
                    <h3 style={{ color: '#6b21a8', margin: '12px 0 6px', fontSize: 18 }}>
                      All Bills Fully Covered by Advance Payment
                    </h3>
                    <p style={{ color: '#7c3aed', fontSize: 14, margin: 0 }}>
                      No payment details needed. Click Submit to update records.
                    </p>
                    <p style={{ color: '#9333ea', fontSize: 12, marginTop: 8 }}>
                  ...
                  <button disabled={submitting} onClick={handleSubmit} style={{
                    ...
                    padding: '14px 40px', borderRadius: 8, border: 'none', width: '100%',
                    background: submitting ? T.border : `linear-gradient(135deg, ${T.purple}, #6d28d9)`,
                    color: submitting ? T.textMuted : '#fff',
                    fontSize: 16, fontWeight: 700,
                    cursor: submitting ? 'not-allowed' : 'pointer',
                    boxShadow: submitting ? 'none' : `0 2px 8px ${T.purple}40`,
                  }}>
                    {submitting ? (
                      <><Loader2 size={16} style={{ animation: 'spin 0.8s linear infinite' }} /> Submitting...</>
                    ) : (
                      <><CheckCircle size={16} /> SUBMIT ADVANCE PAYMENT</>
                    )}
                  </button>
                </>
              ) : (
                <>
                  {/* ✅ NORMAL / MIXED BILLS - Show all fields */}
                  <h3 style={{ marginBottom: 16, color: T.navy, fontSize: 16 }}>
                    Payment Details ({selectedBills.length} Bill{selectedBills.length > 1 ? 's' : ''})
                  </h3>
                  ...
                  <div style={{
                    ...
                    <div style={{ position: 'relative' }} ref={bankRef}>
                      <label style={{ fontSize: 12, fontWeight: 600, color: T.navyLight, display: 'block', marginBottom: 4 }}>
                        ...
                        {accountLoading && <Loader2 size={10} color={T.gold} style={{ marginLeft: 6, animation: 'spin 1s linear infinite', display: 'inline-block', verticalAlign: 'middle' }} />}
                      </label>
                      <div style={{ position: 'relative' }}>
                        <input type="text" placeholder={accountLoading ? "Loading..." : "Search Bank..."}
                          value={bankSearch} disabled={accountLoading}
                          onFocus={() => setShowBankDropdown(true)}
                          onChange={(e) => { setBankSearch(e.target.value); setShowBankDropdown(true); if (e.target.value === "") setBankDetails(""); }}
                          style={{ ...inputBase, paddingRight: 36, borderColor: bankDetails ? T.gold : T.border, background: bankDetails ? `${T.gold}08` : T.borderLight }} />
                        <ChevronDown size={14} style={{ position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)', color: T.textMuted, pointerEvents: 'none' }} />
                      </div>
                      {bankDetails && (
                        <div style={{ marginTop: 4, display: 'flex', alignItems: 'center', gap: 6 }}>
                          <span style={{ fontSize: 11, color: '#fff', background: T.navy, padding: '2px 8px', borderRadius: 4, fontWeight: 600 }}>✓ {bankDetails}</span>
                          <button onClick={() => { setBankDetails(""); setBankSearch(""); }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: T.danger, fontSize: 11, display: 'flex', alignItems: 'center' }}><X size={11} /></button>
                        </div>
                      )}
                      {showBankDropdown && !accountLoading && (
                        <ul style={{
                          position: 'absolute', top: '100%', left: 0, right: 0, background: T.card,
                          border: `1px solid ${T.border}`, borderRadius: 8, marginTop: 4, maxHeight: 220,
                          overflowY: 'auto', zIndex: 200, listStyle: 'none', padding: 0, boxShadow: '0 8px 24px rgba(0,0,0,0.12)',
                        }}>
                          <li style={{ padding: '6px 14px', fontSize: 11, color: T.textMuted, borderBottom: `1px solid ${T.borderLight}`, background: T.borderLight }}>
                            {accountNames.filter(n => n.toLowerCase().includes(bankSearch.toLowerCase())).length} accounts
                          </li>
                          {accountNames.filter(n => n.toLowerCase().includes(bankSearch.toLowerCase())).map((name, i) => (
                            <li key={i}
                              onMouseDown={(e) => { e.preventDefault(); setBankDetails(name); setBankSearch(name); setShowBankDropdown(false); }}
                              style={{
                                padding: '10px 14px', cursor: 'pointer', borderBottom: `1px solid ${T.borderLight}`, fontSize: 13,
                                background: bankDetails === name ? `${T.gold}15` : 'transparent',
                                fontWeight: bankDetails === name ? 700 : 400, display: 'flex', alignItems: 'center', gap: 8,
                              }}
                    ...
                    <div>
                      <label style={{ fontSize: 12, fontWeight: 600, color: T.navyLight, display: 'block', marginBottom: 4 }}>
                        MODE <span style={{ color: T.danger }}>*</span>
                      </label>
                    ...
                    <div>
                      <label style={{ fontSize: 12, fontWeight: 600, color: T.navyLight, display: 'block', marginBottom: 4 }}>
                        {paymentDetailsLabel} <span style={{ color: T.danger }}>*</span>
                      </label>
                      <input type="text" placeholder={paymentMode === "Cheque" ? "Cheque Number" : "Details"}
                        value={paymentDetails} onChange={(e) => setPaymentDetails(e.target.value)} style={inputBase} />
                    </div>
                    ...
                    <div>
                      ...
                      <input type="date" value={paymentDate} onChange={(e) => setPaymentDate(e.target.value)} style={inputBase} />
                    </div>
                  </div>
                  ...
                  <button disabled={submitting} onClick={handleSubmit} style={{
                    ...
                    color: submitting ? T.textMuted : T.navyDark, fontSize: 16, fontWeight: 700,
                    cursor: submitting ? 'not-allowed' : 'pointer',
                    boxShadow: submitting ? 'none' : `0 2px 8px ${T.gold}40`,
                  }}>
                    {submitting ? (
...
const DocLink = ({ label, url }) => (
  <a href={url} target="_blank" rel="noopener noreferrer" style={{
    ...
    background: '#1e293b', color: '#fbbf24', fontSize: 11, fontWeight: 700, textDecoration: 'none',
  }}><FileText size={11} /> {label}</a>
);
