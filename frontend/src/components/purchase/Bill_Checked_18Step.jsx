...
const Bill_Checked_13Step = () => {
  ...
  const [formData, setFormData] = useState({ STATUS_13: '', REMARK_13: '' });
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/Bill_Checked_Step13`);
  ...
  const handleStatusSubmit = async () => {
    if (!formData.STATUS_13 || !formData.REMARK_13) {
    ...
    try {
      const payload = {
        updates: selectedUIDs.map(uid => ({
          uid: String(uid).trim(),
          STATUS_13: formData.STATUS_13,
          REMARK_13: formData.REMARK_13,
        })),
      };
      const res = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/bill_checked_status_Step13`, {
        method: 'POST',
  ...
  const handleCancel = () => {
    ...
    setFormData({ STATUS_13: '', REMARK_13: '' });
  ...
  const tableCols = [
    ...
    { label: 'Status 13', w: 100 },
    { label: 'Remark 13', w: 130 },
    { label: 'Action', w: 80 },
  ];
                ...
                {data.map((r, idx) => (
                  ...
                  >
                    ...
                    <Td>
                      {r.status13 ? (
                        <span style={{
                          background: r.status13 === 'Done' ? `${T.success}15`
                            : r.status13 === 'Pending' ? `${T.danger}15` : T.borderLight,
                          color: r.status13 === 'Done' ? T.success
                            : r.status13 === 'Pending' ? T.danger : T.text,
                          padding: '3px 8px', borderRadius: 6, fontSize: 11, fontWeight: 600,
                        }}>{r.status13}</span>
          ...
          <div style={{
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '20px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                <div>
                  <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: T.navyLight, marginBottom: 6 }}>
                    Status 13 <span style={{ color: T.danger }}>*</span>
                    <span style={{ marginLeft: 8, fontSize: 10, color: T.textMuted, background: T.borderLight, padding: '2px 6px', borderRadius: 4 }}>→ CA</span>
                  </label>
                  <div style={{ position: 'relative' }}>
                    <select value={formData.STATUS_13}
                      onChange={e => setFormData(p => ({ ...p, STATUS_13: e.target.value }))}
                ...
                <div>
                  <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: T.navyLight, marginBottom: 6 }}>
                    Remark 13 <span style={{ color: T.danger }}>*</span>
                    <span style={{ marginLeft: 8, fontSize: 10, color: T.textMuted, background: T.borderLight, padding: '2px 6px', borderRadius: 4 }}>→ CC</span>
                  </label>
                  <textarea value={formData.REMARK_13}
                    onChange={e => setFormData(p => ({ ...p, REMARK_13: e.target.value }))}
            ...
            <div style={{
              ...
              <button onClick={handleStatusSubmit}
                disabled={isSubmitting || !formData.STATUS_13 || !formData.REMARK_13 || saveSuccess}
                style={{
                  display: 'flex', alignItems: 'center', gap: 6,
                  padding: '10px 24px', borderRadius: 8, border: 'none',
                  background: saveSuccess ? T.success
                    : (formData.STATUS_13 && formData.REMARK_13 && !isSubmitting)
                      ? `linear-gradient(135deg, ${T.gold}, ${T.goldDark})` : T.border,
                  color: saveSuccess ? 'white'
                    : (formData.STATUS_13 && formData.REMARK_13 && !isSubmitting) ? T.navyDark : T.textMuted,
                  fontSize: 13, fontWeight: 700,
                  cursor: (formData.STATUS_13 && formData.REMARK_13 && !isSubmitting) ? 'pointer' : 'not-allowed',
                }}>
                {isSubmitting ? (
...
export default Bill_Checked_13Step;