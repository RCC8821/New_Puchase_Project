...
import {
  Loader2, AlertCircle, CheckCircle, X, ChevronDown,
  RotateCcw, Package, FileText, Check, Camera, Filter, Edit3
} from 'lucide-react';
...
const Final_Material_Received = () => {
  ...
  // ✅ Unique UIDs from poItems (duplicates hata do)
  const uniqueUIDs = [...new Set(poItems.map(i => i.uid))];

  const fetchRequests = async () => {
    try {
      setLoading(true); setError(null);
      const response = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/api/get-Final-material-received`
      );
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
      const data = await response.json();
      const items = Array.isArray(data.data) ? data.data : [];
  ...
  // ✅ Row click - unique UIDs pre-select
  const handleRowClick = (req) => {
    if (!req.poNumber) {
      setSaveError('This row has no PO Number');
      return;
    ...
    // ✅ Unique UIDs pre-select
    const uniq = [...new Set(items.map(i => i.uid))];
    setSelectedUIDs(uniq);
  ...
  // ✅ Select All - unique UIDs use karo
  const handleSelectAllUIDs = (e) => {
    setSelectedUIDs(e.target.checked ? uniqueUIDs : []);
  };
  ...
  // ✅ SAVE - Same UID ki entries ka SUM karke bhejo
  const handleSave = async () => {
    if (selectedUIDs.length === 0) {
      setSaveError('Select at least one UID');
      return;
    ...
    try {
      ...
      // ✅ Har unique UID ke liye process karo
      for (const uid of selectedUIDs) {
        // Is UID ki saari entries (Material_Received mein multiple ho sakti hain)
        const sameUidEntries = poItems.filter(i => i.uid === uid);
        if (sameUidEntries.length === 0) continue;

        // ✅ Same UID ki quantities ka TOTAL
        const totalQtyForThisUID = sameUidEntries.reduce((sum, item) => {
          return sum + (parseFloat(item.totalReceivedQuantity) || 0);
        }, 0);

        // Pehli entry se baaki data lo
        const firstItem = sameUidEntries[0];

        // Saare challans combine karo
        const allChallans = sameUidEntries
          .map(i => i.Challan_url)
          .filter(Boolean);

        const allChallanNos = sameUidEntries
          .map(i => i.challanNo)
          .filter(Boolean)
          .join(', ');
        ...
        const dataToSend = {
          uid: uid,
          totalReceivedQuantity: totalQtyForThisUID, // ✅ SUM
          status: 'Done',
          challan_urls: allChallans,
          challanNo: allChallanNos,
          qualityApproved: firstItem.qualityApproved || 'Final Done',
        };

        console.log(
          `📦 UID ${uid}: ${sameUidEntries.length} entries → Total = ${totalQtyForThisUID}`
        );
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 12, color: T.textMuted, margin: 0 }}>
              {requests.length} pending • Click any row to mark done
            </p>
          </div>
        </div>
        <button onClick={fetchRequests} style={{
        ...
        <div style={{
          ...
          {(selectedVendor || selectedPO) && (
            <div style={{
              ...
              {selectedVendor && <span style={{
                background: `${T.navy}10`, color: T.navy,
                padding: '3px 8px', borderRadius: 6,
                fontSize: 11, fontWeight: 600
              }}>{selectedVendor}</span>}
              {selectedPO && <span style={{
              ...
              <button onClick={clearFilters} style={{
                ...
                color: T.textLight, fontSize: 11, cursor: 'pointer'
              }}>✕ Clear</button>
              <span style={{
                fontSize: 11, color: T.textMuted, marginLeft: 'auto'
              }}>{requests.length} records</span>
            </div>
          )}
        </div>
      )}
              ...
              <thead style={{ position: 'sticky', top: 0, zIndex: 10 }}>
                <tr style={{ background: T.navy }}>
                  {['#', 'UID', 'Timestamp', 'Req No', 'Site', 'Supervisor', 'Material', 'Size', 'Specification', 'SKU', 'Brand', 'Order Qty', 'Received', 'Unit', 'Status', 'Challan', 'Vendor', 'PO No', 'PO PDF', 'Action'].map(h => (
                    <th key={h} style={{
                      padding: '12px 14px',
                      textAlign: h === 'Order Qty' || h === 'Received' ? 'right'
              ...
              <tbody>
                {requests.map((req, idx) => (
                  <tr key={`${req.uid}-${idx}`}
                    ...
                    <Td maxW={110}>{req.supervisorName}</Td>
                    <Td maxW={150} bold>{req.materialName}</Td>
                    <Td>{req.materialSize && <span style={{
                      background: `${T.gold}15`, color: T.goldDark,
                      padding: '2px 8px', borderRadius: 4,
                      fontSize: 11, fontWeight: 600
                    }}>📏 {req.materialSize}</span>}</Td>
                    <Td maxW={140}>
                      {req.materialSpecification ? (
                        <span style={{
                    ...
                    <Td>{req.skuCode && <span style={{
                      fontFamily: 'monospace', fontSize: 11,
                      background: T.borderLight, padding: '2px 6px',
                      borderRadius: 4, color: T.textLight
                    }}>{req.skuCode}</span>}</Td>
                    <Td maxW={110}>{req.brandName && <span style={{
                      background: `${T.purple}15`, color: T.purple,
                      padding: '3px 8px', borderRadius: 5,
                      fontSize: 11, fontWeight: 600
                    }}>{req.brandName}</span>}</Td>
                    <Td right>{req.revisedQuantity || req.orderQty}</Td>
                    <Td right>
                      <span style={{
                        background: parseFloat(req.totalReceivedQuantity) > 0
                          ? `${T.success}15` : T.borderLight,
                        color: parseFloat(req.totalReceivedQuantity) > 0
                          ? T.success : T.textMuted,
                        padding: '3px 10px', borderRadius: 6,
                        fontSize: 12, fontWeight: 700
                      }}>{req.totalReceivedQuantity || 0}</span>
                    </Td>
                    <Td>{req.unitName}</Td>
                    <Td>
                      {req.status && <span style={{
                        background: req.status === 'Full Material' ? `${T.success}15`
                          : req.status === 'Partition' ? `${T.warning}20`
                          : `${T.purple}15`,
                        color: req.status === 'Full Material' ? T.success
                          : req.status === 'Partition' ? '#92400e'
                          : T.purple,
                        padding: '3px 8px', borderRadius: 6,
                        fontSize: 11, fontWeight: 600,
                      }}>{req.status}</span>}
                    ...
                    <td style={{
                      ...
                      {req.Challan_url ? <a href={req.Challan_url}
                        ...
                        }}><Camera size={11} /> View</a> : '—'}
                    </td>
                    <Td maxW={130}>{req.vendorName}</Td>
                    <Td>{req.poNumber && <span style={{
                      background: T.navy, color: T.gold,
                      padding: '3px 10px', borderRadius: 6,
                      fontSize: 11, fontWeight: 700
                    }}>{req.poNumber}</span>}</Td>
                    <td style={{
                      padding: '10px 14px',
                      borderBottom: `1px solid ${T.border}`,
                      textAlign: 'center'
                    }}>
                      {req.pdfUrl6 ? <a href={req.pdfUrl6}
                        target="_blank" rel="noopener noreferrer"
                        onClick={e => e.stopPropagation()}
                        style={{
                          display: 'inline-flex', alignItems: 'center', gap: 3,
                    ...
                    <td style={{
                      ...
                      <button onClick={() => handleRowClick(req)}
                        title={`Mark PO ${req.poNumber} Done`}
                        style={{
                          width: 34, height: 34, borderRadius: 8,
      ...
      {isModalOpen && (
        <>
          ...
          <div style={{
            ...
            width: '95%', maxWidth: 750, background: T.card,
            borderRadius: 14,
            boxShadow: '0 20px 60px rgba(0,0,0,0.2)', zIndex: 101,
            display: 'flex', flexDirection: 'column', maxHeight: '90vh',
          }}>
            <div style={{
              background: T.navy, padding: '14px 18px',
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <div>
                  <h3 style={{
                    ...
                    Mark Final Done — PO: <span style={{ color: T.gold }}>
                      {selectedPOForModal}
                    </span>
                  </h3>
                  <p style={{ fontSize: 11, color: T.textMuted, margin: 0 }}>
                    {selectedUIDs.length} / {uniqueUIDs.length} unique UIDs selected
                    ({poItems.length} total entries)
                  </p>
                </div>
              </div>
              <button onClick={closeModal} disabled={isSaving} style={{
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '18px' }}>
              ...
              {saveSuccess && (
                <div style={{
                  ...
                  All {selectedUIDs.length} UIDs saved! Refreshing...
                </div>
              )}
              {saveError && (
                <div style={{
              ...
              <div style={{
                ...
                marginBottom: 16, borderLeft: `3px solid ${T.success}`
              }}>
                <p style={{
                ...
                <p style={{ fontSize: 12, color: T.textLight, margin: 0 }}>
                  Same UID ki multiple entries auto plus ho jayengi
                </p>
              </div>
              ...
              <div style={{
                ...
                <input type="checkbox" onChange={handleSelectAllUIDs}
                  checked={
                    selectedUIDs.length === uniqueUIDs.length &&
                    uniqueUIDs.length > 0
                  }
                  style={{ width: 18, height: 18 }} />
                <span style={{ fontSize: 13, fontWeight: 600, color: T.navy }}>
                  Select All ({uniqueUIDs.length} unique UIDs)
                </span>
                <span style={{
              ...
              {/* ✅ Unique UIDs list with SUM */}
              {uniqueUIDs.map((uid) => {
                const entriesForUID = poItems.filter(i => i.uid === uid);
                const firstEntry = entriesForUID[0];
                const totalReceivedForUID = entriesForUID.reduce(
                  (sum, item) => sum + (parseFloat(item.totalReceivedQuantity) || 0),
                  0
                );
                const isSelected = selectedUIDs.includes(uid);

                return (
                  <div key={uid} onClick={() => handleUIDToggle(uid)}
                    style={{
                      padding: '10px 12px', marginBottom: 6,
                      borderRadius: 8, cursor: 'pointer',
                      border: `2px solid ${isSelected ? T.success : T.border}`,
                      background: isSelected ? `${T.success}08` : T.card,
                      transition: 'all 0.2s',
                    }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      ...
                      <div style={{ flex: 1 }}>
                        <div style={{
                          ...
                          <span style={{ fontWeight: 600, fontSize: 13 }}>
                            {firstEntry.materialName}
                          </span>

                          {/* ✅ Multiple entries badge */}
                          {entriesForUID.length > 1 && (
                            <span style={{
                              background: `${T.gold}25`, color: T.goldDark,
                              padding: '2px 7px', borderRadius: 5,
                              fontSize: 11, fontWeight: 700
                            }}>
                              🔗 {entriesForUID.length} entries
                            </span>
                          )}

                          {firstEntry.materialSpecification && (
                            <span style={{
                              background: `${T.purple}15`, color: T.purple,
                              padding: '2px 7px', borderRadius: 5,
                              fontSize: 11, fontWeight: 600
                        ...
                        <div style={{
                          ...
                          color: T.textLight, flexWrap: 'wrap'
                        }}>
                          {firstEntry.materialSize && (
                            <span>📏 {firstEntry.materialSize}</span>
                          )}
                          <span>
                            Order: <strong>
                              {firstEntry.revisedQuantity || firstEntry.orderQty} {firstEntry.unitName}
                            </strong>
                          </span>

                          {/* ✅ Total Received (SUM of same UID entries) */}
                          <span>
                            Total Received: <strong style={{
                            ...
                            {entriesForUID.length > 1 && (
                              <span style={{
                                ...
                                ({entriesForUID.map(e => e.totalReceivedQuantity || 0).join(' + ')})
                              </span>
                            )}
                          </span>
                          ...
                          {firstEntry.status && (
                            <span style={{
                              background: firstEntry.status === 'Full Material'
                                ? `${T.success}15` : `${T.warning}20`,
                              color: firstEntry.status === 'Full Material'
                                ? T.success : '#92400e',
                              padding: '1px 6px', borderRadius: 4, fontWeight: 600,
                            }}>{firstEntry.status}</span>
                          )}
                        </div>
                      </div>
                    </div>
            ...
            <div style={{
              ...
              <button onClick={handleSave}
                disabled={isSaving || saveSuccess || selectedUIDs.length === 0}
                style={{
                  display: 'flex', alignItems: 'center', gap: 6,
                  padding: '10px 22px', borderRadius: 8, border: 'none',
                  background: saveSuccess ? T.success
                    : (selectedUIDs.length > 0 && !isSaving)
                  ...
                  fontSize: 13, fontWeight: 700,
                  cursor: (selectedUIDs.length > 0 && !isSaving && !saveSuccess)
                    ? 'pointer' : 'not-allowed',
                }}>
                {isSaving ? <>
                  <Loader2 size={15} style={{ animation: 'spin 0.8s linear infinite' }} />
                  Saving {selectedUIDs.length}...
                </>
                  : saveSuccess ? <><CheckCircle size={15} /> Saved!</>
                    : <><Check size={15} /> Save ({selectedUIDs.length} UIDs)</>}
              </button>
