...
import {
  Loader2, Lock, Table, Plus, Pencil, X, AlertCircle, Send
} from 'lucide-react';
...
const LabourRequirementManagement = () => {
  ...
  // Robust head detect (sirf Contractor field enable/disable ke liye)
  const isCompanyHeadModal = useMemo(() => {
    if (!selectedItem) return false;
    const rawHead =
      selectedItem.Head_Of_Contractor_Company_1 ||
      selectedItem.Head_Of_Contractor_Company ||
      '';
    const formattedHead = String(rawHead).trim().toLowerCase().replace(/\s+/g, ' ');
    return (
      formattedHead === 'company head' ||
      formattedHead === 'company' ||
      formattedHead.includes('company head')
    );
  }, [selectedItem]);
  ...
  // ── Auto Calculate (Modal Form) ──
  // ✅ FIX: Company Head me hamesha Total Paid aayega (head type se farak nahi padta)
  useEffect(() => {
    if (isRejected) return;
    ...
    setModalForm(prev => ({
      ...
      // ✅ HAMESHA Company Head = Total Paid
      Company_Head_Amount_3: totalPaidStr,

      // Contractor Head: Company type par empty, warna auto-fill (editable)
      Contractor_Head_Amount_3: isCompanyHeadModal ? '' : totalPaidStr,
    }));
  }, [
    modalForm.Number_Of_Labour_1_3,
    modalForm.Labour_Rate_1_3,
    modalForm.Number_Of_Labour_2_3,
    modalForm.Labour_Rate_2_3,
    modalForm.Conveyanance_3,
    modalForm.Contractor_Commission,
    isRejected,
    isCompanyHeadModal,
  ]);
  ...
  const openModal = (item) => {
    ...
    setModalForm({
      Status_3: '',
      Labouar_Contractor_Name_3: '',
      Labour_Category_1_3: item.Labour_Category_1 || '',
      Number_Of_Labour_1_3: item.Number_Of_Labour_1 || '',
      Labour_Rate_1_3: '',
      Labour_Category_2_3: item.Labour_Category_2 || '',
      Number_Of_Labour_2_3: item.Number_Of_Labour_2 || '',
      Labour_Rate_2_3: '',
      Total_Wages_3: '',
      Conveyanance_3: '',
      Contractor_Commission: '',
  ...
  const validateModal = () => {
    ...
    // Company Head amount hamesha auto hoti hai, isliye required nahi
    // Contractor Head sirf non-company case me required
    if (!isCompanyHeadModal && (!modalForm.Contractor_Head_Amount_3 || !String(modalForm.Contractor_Head_Amount_3).trim())) {
      setFormError('Contractor Head Amount is Required');
      return false;
    }
  ...
  const handleModalSubmit = async () => {
    ...
    try {
      let payload = {
        ...
        Status_3: modalForm.Status_3,
        Remark_3: modalForm.Remark_3 || ''
      };
          ...
          <div style={{ background: T.card, borderRadius: 14, border: `1px solid ${T.border}`, overflow: 'hidden' }}>
            <div style={{ background: `linear-gradient(135deg, ${T.navy}, ${T.navyDark})`, padding: '20px 24px', borderBottom: `3px solid ${T.gold}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <button
                ...
                style={{ padding: '8px 16px', borderRadius: 8, background: `${T.gold}20`, color: T.gold, border: `1px solid ${T.gold}60`, cursor: 'pointer' }}
              >
                Refresh
              </button>
            </div>
            ...
            <div style={{ padding: '16px 24px' }}>
              <input
                ...
                style={{ width: '100%', maxWidth: 400, padding: '10px 12px', borderRadius: 8, border: `1.5px solid ${T.border}`, outline: 'none' }}
              />
            </div>
                      ...
                      <tr key={row.uid || idx} style={{ background: idx % 2 === 0 ? T.card : T.borderLight }}>
                        ...
                        <td style={{ padding: '10px', textAlign: 'center' }}>
                          <button
                            ...
                            style={{ width: 34, height: 34, borderRadius: 8, border: `1.5px solid ${T.gold}40`, background: `${T.gold}10`, color: T.goldDark, cursor: 'pointer' }}
                          >
                            <Pencil size={15} />
                          </button>
                        </td>
            ...
            <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: '95%', maxWidth: 600, background: T.card, borderRadius: 14, boxShadow: '0 20px 60px rgba(0,0,0,0.2)', zIndex: 101, display: 'flex', flexDirection: 'column', maxHeight: '92vh' }}>
              ...
              <div style={{ background: T.navy, padding: '14px 20px', borderBottom: `2px solid ${T.gold}`, display: 'flex', justifyContent: 'space-between', borderRadius: '14px 14px 0 0' }}>
                <div>
                  ...
                  <p style={{ fontSize: 11, color: T.textMuted, margin: 0 }}>
                    UID: <span style={{ color: T.gold }}>{selectedItem.uid}</span> | {selectedItem.Project_Name_1}
                  </p>
                </div>
                <button onClick={() => setShowModal(false)} style={{ background: 'transparent', border: 'none', color: 'white', cursor: 'pointer' }}>
                  <X size={20} />
                </button>
              </div>
              ...
              <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 14 }}>
                <div>
                  ...
                  <select
                    value={modalForm.Status_3}
                    onChange={(e) => handleModalChange('Status_3', e.target.value)}
                  ...
                  <>
                    ...
                    <div style={{ padding: 14, background: `${T.gold}08`, borderRadius: 10, border: `1px dashed ${T.gold}50` }}>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
                        <div>
                          ...
                          <input type="text" value={modalForm.Labour_Category_1_3} disabled style={{ ...inputBase, background: T.borderLight }} />
                        </div>
                        <div>
                          <label style={{ display: 'block', fontSize: 11, color: T.navyLight, marginBottom: 4 }}>No. of Labour</label>
                          <input type="number" value={modalForm.Number_Of_Labour_1_3} onChange={(e) => handleModalChange('Number_Of_Labour_1_3', e.target.value)} style={inputBase} />
                        </div>
                        <div>
                          <label style={{ display: 'block', fontSize: 11, color: T.navyLight, marginBottom: 4 }}>Rate (₹)</label>
                          <input type="number" value={modalForm.Labour_Rate_1_3} onChange={(e) => handleModalChange('Labour_Rate_1_3', e.target.value)} style={inputBase} />
                        </div>
                      </div>
                    </div>
                    ...
                    <div style={{ padding: 14, background: `${T.navy}08`, borderRadius: 10, border: `1px dashed ${T.navy}30` }}>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
                        <div>
                          <label style={{ display: 'block', fontSize: 11, color: T.navyLight, marginBottom: 4 }}>Category</label>
                          <input type="text" value={modalForm.Labour_Category_2_3} disabled style={{ ...inputBase, background: T.borderLight }} />
                        </div>
                        <div>
                          <label style={{ display: 'block', fontSize: 11, color: T.navyLight, marginBottom: 4 }}>No. of Labour</label>
                          <input type="number" value={modalForm.Number_Of_Labour_2_3} onChange={(e) => handleModalChange('Number_Of_Labour_2_3', e.target.value)} style={inputBase} />
                        </div>
                        <div>
                          <label style={{ display: 'block', fontSize: 11, color: T.navyLight, marginBottom: 4 }}>Rate (₹)</label>
                          <input type="number" value={modalForm.Labour_Rate_2_3} onChange={(e) => handleModalChange('Labour_Rate_2_3', e.target.value)} style={inputBase} />
                        </div>
                      </div>
                    </div>
                    ...
                    <div style={{ padding: 14, background: T.successBg, borderRadius: 10, border: `1px solid ${T.successBorder}` }}>
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                        <div>
                          <label style={{ display: 'block', fontSize: 11, color: T.navyLight, marginBottom: 4 }}>Total Wages (Auto)</label>
                          <div style={{ padding: '8px 12px', background: T.card, border: `1px solid ${T.border}`, borderRadius: 8, fontSize: 13, fontWeight: 700, color: T.success }}>
                            ₹ {modalForm.Total_Wages_3 || '0'}
                          </div>
                        </div>
                        <div>
                          <label style={{ display: 'block', fontSize: 11, color: T.navyLight, marginBottom: 4 }}>Conveyance (₹)</label>
                          <input type="number" value={modalForm.Conveyanance_3} onChange={(e) => handleModalChange('Conveyanance_3', e.target.value)} style={inputBase} />
                        </div>
                        <div style={{ gridColumn: '1 / -1' }}>
                          <label style={{ display: 'block', fontSize: 11, color: T.navyLight, marginBottom: 4 }}>Commission (₹)</label>
                          <input type="number" value={modalForm.Contractor_Commission} onChange={(e) => handleModalChange('Contractor_Commission', e.target.value)} style={{ ...inputBase, border: `1.5px solid ${T.gold}` }} />
                        </div>
                        <div style={{ gridColumn: '1 / -1' }}>
                          <label style={{ display: 'block', fontSize: 11, color: T.navyLight, marginBottom: 4 }}>Total Paid (Auto)</label>
                          <div style={{ padding: '12px', background: `${T.success}15`, border: `2px solid ${T.success}40`, borderRadius: 8, fontSize: 18, fontWeight: 800, color: T.success, textAlign: 'center' }}>
                            ₹ {modalForm.Total_Paid_Amount_3 || '0'}
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div style={{ padding: 14, background: `${T.purple}08`, borderRadius: 10, border: `1px dashed ${T.purple}40` }}>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                        <div>
                          <label style={{ display: 'block', fontSize: 11, color: T.navyLight, marginBottom: 4 }}>
                            🏢 Company Head <span style={{ color: T.success }}>(Auto = Total Paid)</span>
                          </label>
                          <input
                            type="number"
                            value={modalForm.Company_Head_Amount_3}
                            disabled
                            style={{
                              ...inputBase,
                        ...
                        <div>
                          <label style={{ display: 'block', fontSize: 11, color: T.navyLight, marginBottom: 4 }}>
                            👷 Contractor Head {!isCompanyHeadModal && <span style={{ color: T.danger }}>* (Editable)</span>}
                          </label>
                          <input
                            type="number"
                            value={modalForm.Contractor_Head_Amount_3}
                            onChange={(e) => handleModalChange('Contractor_Head_Amount_3', e.target.value)}
                            disabled={isCompanyHeadModal}
                            placeholder={isCompanyHeadModal ? 'N/A' : '0'}
                            style={{
                              ...inputBase,
                              background: isCompanyHeadModal ? T.borderLight : T.card,
                              fontWeight: 700,
                              color: !isCompanyHeadModal ? T.purple : T.textMuted,
                              cursor: isCompanyHeadModal ? 'not-allowed' : 'text',
                              borderColor: !isCompanyHeadModal ? T.purple : T.border
                            }}
                          />
                        </div>
                      </div>
                ...
                <div>
                  ...
                  <textarea
                    value={modalForm.Remark_3}
                    onChange={(e) => handleModalChange('Remark_3', e.target.value)}
                    rows={3}
                ...
                {formError && (
                  <div style={{ padding: '10px', background: T.dangerBg, color: T.danger, borderRadius: 8, fontSize: 13, display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <div style={{ padding: '14px 20px', borderTop: `1px solid ${T.border}`, background: T.borderLight, borderRadius: '0 0 14px 14px', display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
                <button onClick={() => setShowModal(false)} style={{ padding: '10px 20px', borderRadius: 8, border: `1.5px solid ${T.border}`, background: T.card, cursor: 'pointer' }}>
                  Cancel
                </button>
                <button
                  onClick={handleModalSubmit}
                  disabled={isUpdatingModal || !modalForm.Status_3}
                  style={{
                    padding: '10px 24px', borderRadius: 8, border: 'none',
                    background: `linear-gradient(135deg, ${T.gold}, ${T.goldDark})`,
                    color: T.navyDark, fontWeight: 700, cursor: 'pointer',
...
const FormFields = ({
  ...
  isContractorHead, isDropdownLoading, getYesterdayDate
}) => {
  const labelStyle = { display: 'block', fontSize: 12, fontWeight: 600, color: T.navy, marginBottom: 6 };
  const disabledInput = { ...inputBase, background: T.borderLight, cursor: 'not-allowed', color: T.textLight };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ padding: 14, background: `${T.gold}08`, borderRadius: 10, border: `1px dashed ${T.gold}60` }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
          <div>
            <label style={labelStyle}>Category 1 *</label>
            <select value={data.Labour_Category_1} onChange={(e) => onChange('Labour_Category_1', e.target.value)} style={inputBase}>
              <option value="">-- Select --</option>
          ...
          <div>
            ...
            <input type="number" min="0" value={data.Number_Of_Labour_1} onChange={(e) => onChange('Number_Of_Labour_1', e.target.value)} style={inputBase} />
          </div>
          <div>
            <label style={labelStyle}>Category 2</label>
            <select value={data.Labour_Category_2} onChange={(e) => onChange('Labour_Category_2', e.target.value)} style={inputBase}>
              <option value="">-- Select --</option>
              {labourCategoryOptions.map((opt, i) => <option key={i} value={opt}>{opt}</option>)}
            </select>
          </div>
          <div>
            ...
            <input type="number" min="0" value={data.Number_Of_Labour_2} onChange={(e) => onChange('Number_Of_Labour_2', e.target.value)} style={inputBase} />
          </div>
        </div>
        <div style={{ marginTop: 12, padding: 12, background: T.card, borderRadius: 8, border: `1px solid ${T.gold}40`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <span style={{ fontSize: 12, fontWeight: 700, color: T.navy }}>Total Labour (Auto)</span>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: isContractorHead ? '1fr 1fr 1fr' : '1fr', gap: 16 }}>
        <div>
          ...
          <select value={data.Head_Of_Contractor_Company_1} onChange={(e) => onHeadChange(e.target.value)} style={inputBase}>
            <option value="">-- Select --</option>
        ...
        {isContractorHead && (
          <>
            <div>
              ...
              <select value={data.Name_Of_Contractor_1} onChange={(e) => onContractorChange(e.target.value)} style={inputBase}>
                <option value="">-- Select --</option>
                {contractorOptions.map((opt, i) => <option key={i} value={opt.value}>{opt.label}</option>)}
              </select>
            </div>
            <div>
              <label style={labelStyle}>Firm (auto)</label>
      ...
      <div>
        ...
        <textarea rows={2} value={data.Remark_1} onChange={(e) => onChange('Remark_1', e.target.value)} style={{ ...inputBase, resize: 'vertical' }} />
      </div>
    </div>
  );
};
