...
//   // Check user types
//   const isSignatureReqUser = currentUserType === 'Signature Requirement';
//   const isProjectLocked = !!projectLockedName || isSignatureReqUser;

//   // ✅ Get the locked project name
//   const lockedProjectName = projectLockedName || (isSignatureReqUser ? 'Signature Heritage PRJ024' : null);
...
//   // ═══════════════════════════════════════════════
//   // ✅ AUTO-SET PROJECT NAME (based on user type)
//   // ═══════════════════════════════════════════════
//  // ✅ Auto-set project (for locked users)
// useEffect(() => {
//   if (!apiData) return;

//   if (isProjectLocked && lockedProjectName) {
//     setFormData(prev => ({
//       ...prev,
//       projectName: lockedProjectName,
//     }));
//   }
// }, [isProjectLocked, lockedProjectName, apiData]);

// // ✅ NEW - Auto-fill Engineer when project changes
// useEffect(() => {
//   if (!formData.projectName || !apiData) return;

//   const engineers = maps.projectToEngineers?.[formData.projectName.toLowerCase()] || [];

//   // ✅ Agar engineers hain aur abhi koi engineer selected nahi hai — pehla engineer set karo
//   if (engineers.length > 0 && !formData.engineerName) {
//     setFormData(prev => ({
//       ...prev,
//       engineerName: engineers[0], // First engineer auto-selected
//     }));
//   }
// }, [formData.projectName, apiData]);


//   // ✅ Project change handler
// const handleProjectChange = (val) => {
//   // ✅ Auto-fill first engineer of the new project
//   const engineers = maps.projectToEngineers?.[val.toLowerCase()] || [];
//   const firstEngineer = engineers.length > 0 ? engineers[0] : '';

//   setFormData(prev => ({
//     ...prev,
//     projectName: val,
//     engineerName: firstEngineer, // ✅ Auto-fill first engineer
//   }));
// };

//   // ✅ Engineer change handler
...
//   // Material Type change → Reset name, size, spec, sku, unit
...
//   // Material Size change → Auto-fill SKU + Unit (from SKU)
...
//   // ✅ Reset - Keep locked project
//   const resetForm = () => {
//     setFormData({
//       projectName: isProjectLocked ? lockedProjectName : '',
//       engineerName: '',
//       contractor: '',
//       remark: '',
//     });
...
//               <strong style={{ color: T.goldDark }}>{lockedProjectName}</strong>
//             </div>
//           </div>
//         </div>
//       )}
...
//           <div>
//             <label style={S.label}>
//               Store Name <span style={S.req}>*</span>
    ...
    <div style={{ position: 'relative', width: '100%' }}>
      ...
      <div style={{ position: 'relative' }}>
        <input
          ...
          onFocus={() => { setIsOpen(true); setSearch(""); }}
          onBlur={() =>
            setTimeout(() => { setIsOpen(false); setSearch(""); }, 200)
          }
          disabled={disabled}
          placeholder={placeholder}
          style={{
      ...
      {isOpen && !disabled && (
        <ul style={{
          ...
          {!search && filtered.length > MAX && (
            <li style={{
              padding: '8px', fontSize: 11, color: T.textMuted,
...
const HeritageRequirementForm = () => {
  ...
  const lockedProjectName =
    projectLockedName ||
    (isSignatureReqUser ? 'Signature Heritage PRJ024' : null);
  ...
  // ✅ emptyItem with brand field
  const emptyItem = {
    materialType: '',
    materialName: '',
    materialSize: '',
    specification: '',
    brand: '',       // ✅ Brand - Optional - Column J
    skuCode: '',
    quantity: '',
    unit: '',
    description: '',
  ...
  // ─── AUTO-SET PROJECT (locked users) ─────────────────
  useEffect(() => {
    if (!apiData) return;
    if (isProjectLocked && lockedProjectName) {
      setFormData(prev => ({ ...prev, projectName: lockedProjectName }));
    }
  }, [isProjectLocked, lockedProjectName, apiData]);
  ...
  useEffect(() => {
    if (!formData.projectName || !apiData) return;
    const engineers =
      maps.projectToEngineers?.[formData.projectName.toLowerCase()] || [];
    if (engineers.length > 0 && !formData.engineerName) {
      setFormData(prev => ({ ...prev, engineerName: engineers[0] }));
    }
  }, [formData.projectName, apiData]);
  ...
  const handleProjectChange = (val) => {
    const engineers =
      maps.projectToEngineers?.[val.toLowerCase()] || [];
    const firstEngineer = engineers.length > 0 ? engineers[0] : '';
    setFormData(prev => ({
      ...prev,
      projectName: val,
      engineerName: firstEngineer,
    }));
  };

  const handleEngineerChange = (val) => {
    setFormData(prev => ({ ...prev, engineerName: val }));
  };
  ...
  const handleItemChange = (index, field, value) => {
    ...
    // ✅ Material Type → Reset below fields + brand
    if (field === 'materialType') {
      updated[index] = {
        ...updated[index],
        materialType: value,
    ...
    // ✅ Material Name → Reset below fields + brand
    if (field === 'materialName') {
      updated[index] = {
        ...updated[index],
        materialName: value,
    ...
    // ✅ Material Size → Auto-fill SKU + Unit
    if (field === 'materialSize') {
      const nameKey = updated[index].materialName.toLowerCase();
      const sizeKey = value.toLowerCase();
      const comboKey = `${nameKey}|||${sizeKey}`;
      ...
      if (foundSKU) {
        updated[index].unit =
          maps.skuCodeToUnit?.[foundSKU.toLowerCase()] || '';
      } else {
        updated[index].unit = '';
      }
    }
        ...
        html: `
          <div style="text-align: left; padding: 12px 0;">
            <div style="background: #f8fafc; padding: 12px; border-radius: 8px;
              margin-bottom: 10px; border-left: 4px solid ${T.gold};">
              <p style="margin: 6px 0; font-size: 14px; color: #334155;">
                <strong>Req No:</strong>
                <span style="color: ${T.goldDark}; font-weight: 700;
                  margin-left: 8px;">${res.reqNo}</span>
              </p>
              <p style="margin: 6px 0; font-size: 14px; color: #334155;">
                <strong>Total Items:</strong>
                <span style="color: ${T.success}; font-weight: 700;
                  margin-left: 8px;">${res.itemCount}</span>
              </p>
            </div>
            <p style="margin: 6px 0; font-size: 12px; color: #64748b;
              text-align: center;">${res.message}</p>
          </div>
        `,
        confirmButtonText: 'OK',
        confirmButtonColor: T.gold,
  ...
  // ✅ Reset - Keep locked project + brand cleared
  const resetForm = () => {
    setFormData({
      projectName: isProjectLocked ? lockedProjectName : '',
      engineerName: '',
  ...
  // ── Progress (brand optional - not counted) ──
  // 2 formData + 9 per item
  const totalRequired = 2 + (items.length * 9);
  const filledCount = (() => {
    let count = 0;
    if (formData.projectName.trim()) count++;
    ...
    <div style={{ maxWidth: 1100, margin: '0 auto', padding: '0 8px' }}>
      ...
      {isProjectLocked && (
        <div style={{
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: T.navyDark, marginBottom: 2 }}>
              Project Locked
            </div>
            ...
            <div>
              ...
              <input
                ...
                style={{
                  ...
                  borderLeft: `3px solid ${T.gold}`,
                  background: `${T.gold}08`,
                  color: T.navy, fontWeight: 600, cursor: 'not-allowed',
                }}
              />
            </div>
          ) : (
        ...
        {items.map((item, idx) => {
          ...
          return (
            <div key={idx} style={{
              ...
              {/* ── Row 3: Qty, Unit, ReqDays, Description ── */}
              <div style={{
                display: 'grid',
                gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))',
                gap: 12,
        ...
        {!isFormValid ? (
          <div style={{ margin: 0, flex: '1 1 200px' }}>
            ...
            <div style={{
              ...
              borderRadius: 4, background: T.border,
