...
// // ─── SUBMIT (FORM_DATA!A:Q) - Require Days REMOVED ──────
// router.post('/submit-requirement', async (req, res) => {
//   try {
//     const {
//       projectName,
...
//     const values = items.map((item, i) => {
//       // ✅ Item validation (location per item + no reqDays)
//       if (!item.location) throw new Error(`Item ${i + 1}: Location is required`);
//       if (!item.materialType || !item.materialName ||
//           !item.materialSize || !item.specification ||
//           !item.skuCode || !item.quantity || !item.unit ||
...
//     await sheets.spreadsheets.values.append({
//       spreadsheetId: SignatureSheetId,
//       range: 'FORM_DATA!A:Q',   // ✅ Q tak (Require Days column removed)
//       valueInputOption: 'USER_ENTERED',
//       resource: { values },
//     });
...
// // ─── GET SITE ENGINEER DATA (Filtered by Engineer Name) ──
...
//       }))
//       .filter(item => {
//         // ✅ Common filters (S has value, T is empty)
//         if (item.sColumn === '' || item.tColumn !== '') return false;

//         // ✅ Admin sees all data
//         if (isAdmin) return true;

//         // ✅ Site Engineer sees only their data
...
//     // ✅ Update U, V, W columns for specific row
//     await sheets.spreadsheets.values.update({
//       spreadsheetId: SignatureSheetId,
//       range: `FORM_DATA!U${rowNumber}:W${rowNumber}`,
//       valueInputOption: 'USER_ENTERED',
//       resource: {
  ...
  try {
    ...
    const filteredData = rows
      .map((row, index) => ({
        ...
        uid: (row[1] || '').trim(),              // B
        reqNo: (row[2] || '').trim(),            // C
        projectName: (row[3] || '').trim(),      // D
        engineerName: (row[4] || '').trim(),     // E
        cluster: (row[5] || '').trim(),          // F
        location: (row[6] || '').trim(),         // G
        activity: (row[7] || '').trim(),         // H
        materialType: (row[8] || '').trim(),     // I
        materialName: (row[9] || '').trim(),     // J
        materialSize: (row[10] || '').trim(),    // K
        specification: (row[11] || '').trim(),   // L
        skuCode: (row[12] || '').trim(),         // M
        qty: (row[13] || '').trim(),             // N
        unit: (row[14] || '').trim(),            // O
        description: (row[15] || '').trim(),     // P
        remark: (row[16] || '').trim(),          // Q
        plannedDate: (row[18] || '').trim(),     // R
        sColumn: (row[18] || '').trim(),         // S (Filter check)
        tColumn: (row[19] || '').trim(),         // T (Filter check)
        existingStatus: (row[20] || '').trim(),   // U
        existingQuantity: (row[21] || '').trim(), // V
        existingRemarks: (row[22] || '').trim(),  // W
      }))
      .filter(item => {
        // ✅ Common filters (S has value, T is empty)
        if (item.sColumn === '' || item.tColumn !== '') return false;
