    ...
    const inventory = rows
      ...
      .map((row, index) => {
        ...
        return {
          id: index + 1,
          skuCode:              (row[0] || '').trim(),
          materialType:         (row[1] || '').trim(),
          materialName:         (row[2] || '').trim(),
          materialSize:         (row[3] || '').trim(),
          materialSpecification:(row[4] || '').trim(),
          unit:                 (row[5] || '').trim(),
          openingStock:         openingStock,
          outData:              outData,
          stockBalance:         stockBalance,
          availablePercent:     (row[9] || '').trim(),
        };
      });
    ...
    const boqData = rows
      ...
      .map((row, index) => {
        ...
        return {
          ...
          location:             (row[1] || '').trim(),
          activity:             (row[2] || '').trim(),
          materialType:         (row[3] || '').trim(),
          materialName:         (row[4] || '').trim(),
          materialSize:         (row[5] || '').trim(),
          materialSpecification:(row[6] || '').trim(),
          skuCode:              (row[7] || '').trim(),
          outQty:               outQty,
          reviseBOQ:            reviseBOQ,
          balance:              balance,
