...
// // // ─── GET PROJECT DATA from Form_Material (A to J) ───────
// // router.get('/project-data', async (req, res) => {
// //   try {
// //     const response = await sheets.spreadsheets.values.get({
// //       spreadsheetId,
// //       range: 'Form_Material!A2:J',
// //     });
...
// //         contractor: (row[2] || '').trim(),            // C
// //         materialType: (row[3] || '').trim(),          // D
// //         materialName: (row[4] || '').trim(),          // E
// //         materialSize: (row[5] || '').trim(),          // F
// //         unitName: (row[6] || '').trim(),              // G
// //         specification: (row[7] || '').trim(),         // H
// //         brandName: (row[8] || '').trim(),             // I
// //         skuCode: (row[9] || '').trim(),               // J
// //       }));

// //     // ══════════════════════════════════════════════
// //     //  UNIQUE VALUES (Full dropdowns - no filter)
// //     // ══════════════════════════════════════════════
// //     const uniqueValues = {
...
// //       brandNames: [...new Set(projectData.map(d => d.brandName).filter(Boolean))].sort(),
// //       materialTypes: [...new Set(projectData.map(d => d.materialType).filter(Boolean))].sort(),
// //     };

// //     // ══════════════════════════════════════════════
// //     //  MAPS (Dependent dropdowns)
// //     // ══════════════════════════════════════════════

// //     // 1. Project → Engineer (auto-fill, one-to-one)
...
// //     // 3. MaterialName → Sizes (multiple sizes for same name)
// //     const nameToSizes = {};
// //     projectData.forEach(d => {
...
// //     // 4. MaterialName + Size → SKU Code (auto-fill)
// //     //    Key: "materialname|||size"
...
// //     // 5. Size → Specifications (filtered by size)
// //     //    Key: "materialname|||size"
// //     const nameAndSizeToSpecs = {};
// //     projectData.forEach(d => {
// //       if (!d.materialName || !d.materialSize) return;
// //       const key = `${d.materialName.toLowerCase()}|||${d.materialSize.toLowerCase()}`;
// //       if (!nameAndSizeToSpecs[key]) nameAndSizeToSpecs[key] = new Set();
// //       if (d.specification) nameAndSizeToSpecs[key].add(d.specification);
// //     });
// //     Object.keys(nameAndSizeToSpecs).forEach(k => {
// //       nameAndSizeToSpecs[k] = [...nameAndSizeToSpecs[k]].sort();
// //     });
...
// //       uniqueValues,
// //       maps: {
// //         projectToEngineer,     // project → single engineer (auto-fill)
// //         typeToNames,           // matType → [matNames]
// //         nameToSizes,           // matName → [sizes]
// //         nameAndSizeToSKU,      // matName+size → skuCode
// //         nameAndSizeToSpecs,    // matName+size → [specs]
// //       },
// //     });
...
// //       throw new Error('Project Name & Engineer Name are required');
// //     }
...
// //         throw new Error(`Item ${i + 1}: Material Type, Name & Quantity required`);
// //       }
...
// //         currentUID + i,               // B: UID
// //         reqNo,                        // C: Req No
// //         projectName,                  // D: Project_Name_1
// //         engineerName,                 // E: Project_Engineer_Name_1
// //         item.materialType,            // F: Material_Type_1
// //         item.materialName,            // G: Material_Name_1
// //         item.materialSize || '',      // H: Material_Size_1
// //         item.specification || '',     // I: Material_Specification_1
// //         item.brandName || '',         // J: Material_Brand_Name_1
// //         item.skuCode || '',           // K: SKU Code
// //         item.quantity,                // L: Quantity
// //         item.unit || '',              // M: Unit Name
// //         item.description || '',       // N: Description_Of_Work_1
// //         item.reqDays || '',           // O: Require Days_1
// //         contractor || '',             // P: Name_Of_Contractor/Firm_1
// //         remark || '',                 // Q: Remark_1
// //       ];
// //     });
...
//         brandName: (row[9] || '').trim(),           // J
//       }));

//     // ══════════════════════════════════════════════
//     //  UNIQUE VALUES (Full dropdowns)
//     // ══════════════════════════════════════════════
//     const uniqueValues = {
//       projectNames: [...new Set(projectData.map(d => d.projectName).filter(Boolean))].sort(),
//       contractors: [...new Set(projectData.map(d => d.contractor).filter(Boolean))].sort(),
...
//     // 1. Project → Engineer (one-to-one auto-fill)
//     const projectToEngineer = {};
//     projectData.forEach(d => {
//       if (d.projectName && d.engineerName) {
//         const key = d.projectName.toLowerCase();
...
//     // 🆕 4. MaterialName → ALL Specifications (no size filter)
//     const nameToSpecs = {};
//     projectData.forEach(d => {
...
//     // 5. MaterialName + Size → SKU Code (auto-fill)
//     const nameAndSizeToSKU = {};
//     projectData.forEach(d => {
//       if (!d.materialName || !d.materialSize) return;
//       const key = `${d.materialName.toLowerCase()}|||${d.materialSize.toLowerCase()}`;
...
//         typeToNames,
//         nameToSizes,
//         nameToSpecs,           // 🆕 All specs for material name
//         nameAndSizeToSKU,
//       },
//     });
